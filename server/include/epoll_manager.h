...
public:
    ...
    int wait(epoll_event* events, int maxevents, int timeout_ms) {
        ...
        for (auto& [fd, w] : watched_) {
            ...
            if (w.events & EPOLLIN)  FD_SET(fd, &read_fds);
            if (w.events & EPOLLOUT) FD_SET(fd, &write_fds);
        ...
        int count = 0;
        for (auto& [fd, w] : watched_) {
            if (!w.active || count >= maxevents) break;
            uint32_t revents = 0;
            if (FD_ISSET(fd, &read_fds))  revents |= EPOLLIN;
            if (FD_ISSET(fd, &write_fds)) revents |= EPOLLOUT;
