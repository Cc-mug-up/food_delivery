...
public:
    ...
    Value(double v)      : type_(Type::Double), double_val_(v) { if (std::floor(v) == v && std::abs(v) < 1LL<<53) { int_val_ = (int64_t)v; } }
    ...
    Value& operator[](const std::string& key) {
        if (type_ != Type::Object) { type_ = Type::Object; object_val_.clear(); }
    ...
    size_t size() const {
        if (type_ == Type::Array)  return array_val_.size();
        if (type_ == Type::Object) return object_val_.size();
        return 0;
    }

    // ---- Array 操作 ----
    Value& operator[](size_t idx) {
        if (idx >= array_val_.size()) array_val_.resize(idx + 1);
        return array_val_[idx];
    }
    void push_back(const Value& v) {
        if (type_ != Type::Array) { type_ = Type::Array; array_val_.clear(); }
...
private:
    void serialize(std::ostringstream& oss, int indent, int depth) const {
        ...
        case Type::Bool:   oss << (bool_val_ ? "true" : "false"); break;
        case Type::Int:    oss << int_val_; break;
        case Type::Double: oss << double_val_; break;
        case Type::String: serialize_string(oss, string_val_); break;
        case Type::Array:
            oss << "[" << nl;
...
private:
    ...
    void expect(char c) {
        skip_ws();
        if (next() != c) throw std::runtime_error("JSON: expected '" + std::string(1, c) + "' at pos " + std::to_string(pos_));
    }
    ...
    Value parse_number() {
        ...
        size_t start = pos_;
        if (peek() == '-') next();
        while (pos_ < text_.size() && std::isdigit(peek())) next();
        bool is_float = false;
        if (pos_ < text_.size() && peek() == '.') { is_float = true; next(); }
        while (pos_ < text_.size() && std::isdigit(peek())) next();
        ...
        if (is_float) return std::stod(num);
